...
function Chatroom({ workspaceId, setIsChatOpen }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (error) => {
        console.error("Error subscribing to workspace messages:", error);
        setLoading(false);
      }
    );
  ...
  const generateAIResponse = async (prompt) => {
    ...
    try {
      ...
      return "Sorry, I couldn't process that request: " + (error.message || "Please try again.");
    } finally {
  ...
  const sendMessage = async () => {
    ...
    const imageUrl = user?.photoURL || "/robotic.png";
    const senderId = user?.uid || "anonymous";
    ...
    try {
      ...
      if (aiPrompt) {
        // CC-009: Server-side route creates and persists the authorized AI_BOT message to Firestore
