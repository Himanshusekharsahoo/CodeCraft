...
export async function POST(request, context) {
  try {
    ...
    const switchResult = await GitService.switchBranch(workspaceId, body.branchName, {
      createNew: Boolean(body.createNew),
    });

    // Update Firestore to mirror the newly checked out branch's files
    await syncWorkingTreeToFirestore(workspaceId, user?.token);

    return jsonSuccess({ branch: switchResult });
  } catch (error) {
